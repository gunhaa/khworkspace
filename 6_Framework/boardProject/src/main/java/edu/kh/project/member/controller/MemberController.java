...
public class MemberController {
	...
	public String login(Member inputMember , Model model
		...
		// -> Request scope 
		// -> @SessionAttributes와 함께 사용 시 Session scope
		...
		if(loginMember!=null) { // 로그인 성공 시
			 ...
			 *   
			 * - 서버로 전달된 쿠키에 값 추가, 수정, 삭제 등을 진행 한 후
			 *   다시 클라이언트에게 반환
			...
			//ra.addFlashAttribute(); : 잠시 Session에 추가
	...
	// 로그아웃
	@GetMapping("logout")
	public String logout(Model model, @RequestHeader(value="referer") String referer, HttpSession session , SessionStatus status) {
...
//		}
	...
	public String exceptionHandler(Exception e , Model model) {
		...
		// forward 진행
		// -> View Resolver가 prefix, suffix를 붙여서 jsp 경로를 만든다.
		return "common/error";
	}
