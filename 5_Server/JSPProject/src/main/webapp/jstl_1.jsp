...
<body>
    ...
    <!-- webapp 폴더는 서버 구동 시 인터넷에 배포되는 폴더
        -> 인터넷 상에서 수행되어야 되는 코드, 파일 등을 모두 webapp폴더 내부에 저장함 
    -->
     ...
     <pre>
        JSTL 사용하고자 하는 JSP가 있을 경우
        해당 JSP 최상단에 JSTL 라이브러리를 추가하는 지시가 taglib를 작성 해야한다.
	...
	<pre>
		...
		- c:set 태그로 선언된 변수는 EL을 이용해서 출력 할 수 있음 
	
		- c:set 태그 속성
		1) var   : 변수명
		2) value : 대입될 값
		3) scope : 변수의 범위 (기본값 : page)
	...
	<c:if test="${temp > 20}">
	<!-- test에 작성된 조건이 만족(true)할 때만 화면에 출력 됨 -->
	...
	<h3>조건문 - if ~ else if ~ else (c:choose , c:when, c:otherwise)</h3>
