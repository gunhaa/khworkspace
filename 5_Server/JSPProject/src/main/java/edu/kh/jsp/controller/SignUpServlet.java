...
//Tomcat(WAS == Servlet Container)에서 제공한다
...
public class SignUpServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String message = memberId + "님의 가입을 환영 합니다.";
		
		// 클라이언트와 연결된 출력 스트림을 이용해서 응답 화면 출력
...
//		out.print("");
		...
		// --> JSP(눈에 보이는 코드는 HTML, 해석은 Java)
		
		// Servlet -> RequestDispatcher -> forward(req, resp) -> JSP
		//               (요청 발송자)              (전송)
		
		RequestDispatcher dis = req.getRequestDispatcher("/WEB-INF/views/signUpResult.jsp");
															// JSP 경로 작성 시 webapp 폴더를 기준으로 작성!!
		// HttpServletRequest 객체에 message 변수값 추가
