...
public class MypageProfileServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			// 파라미터가 얻어와지지 않는 이유
			// 1. enctype="multipart/form-data" -> 인코딩이 안되어 있어서 파라미터가 인지되지 않음
			// 2. input type="file" -> 파일 형태의 데이터
			// -> multipart/form-data 형식의 요청을 처리할 수 있는 전용 Request 객체가 필요
			
			// --> MultipartRequest 가 필요하다. 라이브러리 써야함 (cos.jar 라이브러리 이용 - http://servlets.com/cos/)
															// 파일 업로드 라이브러리
			...
			// 2-2 memberprofile 폴더 까지의 절대 경로 얻어오기
			HttpSession session = req.getSession(); // session 얻어오는 것은 지장X(사용 가능)
			
			// 최상위 경로 ("/" == webapp폴더) 의 실제 절대 경로를 얻어옴
			...
			// 지정된 파일 변경 정책에 맞게 이름이 바뀐 파일이 저장된다.
...
//			System.out.println(mpReq.getFilesystemName("profileImage"));
			...
			if(result>0) { // 성공
				...
				session.setAttribute("message", "프로필 이미지가 변경 되었습니다.");
