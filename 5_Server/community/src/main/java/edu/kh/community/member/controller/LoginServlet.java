...
public class LoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// * 모든 doPost() 메소드에 인코딩 변경 코드를 작성해야함 .. 귀찮다..
		
		// -> 모든 요청(전달 방식 가리지 않음)시 req, resp의 문자 인코딩을 UTF-8 로 변경
		// -> 필터(Filter)
		...
		try {
			...
			if(loginMember != null) { // 성공
				...
				// 쿠키 객체 생성
				//Cookie c =new Cookie("클라이언트쪽에 저장될 쿠키이름", "쿠키내용");
				...
				if(req.getParameter("saveId") != null) {
					// 쿠키 파일을 30일 동안 유지시킨다.
					c.setMaxAge(60*60*24*30); // 1초 단위 (30일)
				} else {
					// 쿠키파일을 0초동안 유지
				...
				// -> /community로 시작하는 주소에만 쿠키 적용
				
				// 응답 객체를 이용해서 클라이언트로 전달
				resp.addCookie(c); // 코드가 해석되는 순간 바로전달
			...
			//        클           캐셔(Servlet)                클라이언트의 다른 주소로 재요청
			
			
			resp.sendRedirect(req.getContextPath());
			//resp.sendRedirect("/community/login.jsp");
			// req.getContextPath() : 최상위 주소(/community)
