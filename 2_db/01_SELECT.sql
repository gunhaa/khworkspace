...
- 데이터를 조회(SELECT)G하면 조건에 맞는 행들이 조회됨.
...
--           디벨로퍼의 날짜 표기 방법이 년/월/일 로 지정되어 있다.
...
/*
1) 컬럼명 AS별칭 : 별칭에 띄어쓰기X, 특수문자X, 문자O
2) 컬럼명 별칭 : 1번에서 AS만 생략한 것

3) 컬럼명 AS "별칭" : 별칭에 띄어쓰기O, 특수문자O, 문자O
...
-- DISTINCT : 조회 시 칼럼에 포함된 중복 값을 한 번만 표시할 때 사용

--(주의사항)
--1) DISTINCT는 SELECT문에 딱 한번만 작성할 수 있다. 2번 불가능
...
'_' 예시
1) 'A_' : A뒤에 아무거나 한 글자
2) '___A' : A앞에 아무거나 세 글자 (4글자 문자열이면서 A로 끝나야함)
*/
...
-- 문제점 : 와일드 카드 문자(_) 와 패턴에 사용된 일반문자(_)의 모양이 같아서 문제 발생
-- 해결 방법 : ESCAPE OPTION을 이용하여 일반문자로 처리할 '_' , '%' 앞에 아무 특수문자나 붙임
...
-- EMPLOYEE 테이블에서 전화번호가 없는 사원의 사번, 이름 ,전화 번호 조회
...
1 : FROM 테이블명
2 : WHRE 조건식
4 : ORDER BY 컬럼명 | 별침 | 컬럼순서 [정렬 방식(오름/내림)] [NULLS FIRST | LAST]
숫자는 해석 순서
*/
