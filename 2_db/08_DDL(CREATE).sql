...
CREATE TABLE MEMBER(
    ...
    MEMBER_NAME VARCHAR2(30), -- 한글 3BYTE * 10글자 == 30 BYTE
...
INSERT INTO MEMBER MEMBER(MEMBER_ID, MEMBER_PWD, MEMBER_NAME, MEMBER_SSN)
VALUES('MEM3', 'PASS03' , '김길동', '940202-1234567');
COMMIT;

SELECT * FROM MEMBER;

-- 주민등록번호의 데이터 타입을 NUMBER로 지정시 문제점
...
INSERT INTO MEMBER2 VALUES( '김영희' , 0009091234567);

SELECT * FROM MEMBER2;
--> NUMBER 타입 컬럼에 데이터 삽입 시 앞에 0 이 있으면 사라지는 문제점이 있음
...
/*
    ...
    + 데이터의 수정/ 삭제 가능여부 검사등을 목적으로 함
     ...
     제약 조건 종류
     ** 중요 PRIMARY KEY, NOT NULL, UNIQUE ,CHECK, FOREIGN KEY
...
--USER_CONS_COLUMNS : 제약 조건이 걸려 있는 컬럼을 확인하는 딕셔너리뷰
...
--ORA-01400: NULL을 ("HGH"."USER_USED_NN"."USER_NO") 안에 삽입할 수 없습니다
-- 오류 코드
...
CREATE TABLE USER_USED_UK(
    USER_NO NUMBER ,
    --USER_ID VARCHAR2(20) UNIQUE, --컬럼 레벨 제약조건 설정 (제약조건명 미지정)
    --USER_ID VARCHAR2(20) CONSTRAINT USER_ID_U UNIQUE, --컬럼 레벨 제약조건 설정 (제약조건명 지정)
    USER_ID VARCHAR2(20),
    USER_PWD VARCHAR2(30),
    USER_NAME VARCHAR2(30),
    GENDER VARCHAR2(10),
...
INSERT INTO USER_USED_UK
VALUES (1, 'USER01', 'PASS01', '홍길동', '남', '010-1234-1234', 'hong123@kh.or.kr');
--ORA-00001: 무결성 제약 조건(HGH.USER_ID_U)에 위배됩니다
            --> UNIQUE
...
VALUES (1, 'USER01', 'PASS01', '김길동', '남', '010-1234-1234', 'hong123@kh.or.kr');
--오류 보고 -
--ORA-00001: 무결성 제약 조건(HGH.USER_ID_NAME_U)에 위배됩니다
...
--테이블에 대한 식별자(INDENTIFIER) 역할을 함
--NOT NULL + UNIQUE 제약조건을 의미

-- 한 테이블당 한 개만 설정할 수 있음
-- 컬럼 레벨, 테이블 레벨 둘다 설정 가능함
-- 한 개 컬럼에 설정할 수 있고, 여러 개의 컬럼을 묶어서 설정할 수 있음(복합키)
CREATE TABLE USER_USED_PK(
    USER_NO NUMBER /*CONSTRAINT USER_NO_PK*/    /*PRIMARY KEY*/ , --컬럼레벨 PK지정
                    --(제약조건이름)/*(생략가능)*/ 
...
INSERT INTO USER_USED_PK
VALUES (1, 'USER02', 'PASS02', '이영희', '여', '010-5678-1234', 'LEE123@kh.or.kr');
--ORA-00001: 무결성 제약 조건(HGH.USER_NO_PK)에 위배됩니다 == UNIQUE 오류와 같음
--> 기본 키 중복으로 오류 발생

INSERT INTO USER_USED_PK
VALUES (NULL, 'USER02', 'PASS02', '이영희', '여', '010-5678-1234', 'LEE123@kh.or.kr');
--> 기본키가 NULL이므로 오류 
--ORA-01400: NULL을 ("HGH"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
...
INSERT INTO USER_USED_PK2
VALUES (1, 'USER01', 'PASS01', '홍길동', '남', '010-1234-1234', 'hong123@kh.or.kr');
--ORA-00001: 무결성 제약 조건(HGH.USERNO_USERID_PK)에 위배됩니다
...
INSERT INTO USER_USED_PK2
VALUES (NULL, 'USER01', 'PASS01', '홍길동', '남', '010-1234-1234', 'hong123@kh.or.kr');
--ORA-01400: NULL을 ("HGH"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다
--복합키여도 PK이므로 NULL은 절대 안됨!!!

--------------------------------------------------------------------------------

-- 4. FOREIGN KEY ( 외부키/ 외래키) 제약조건
--> 다른 테이블의 PK 또는 UNIQUE 제약 조건이 설정된 컬럼
...
CREATE TABLE USER_USED_FK(
    ...
    PHONE VARCHAR2(30),
    EMAIL VARCHAR2(50),
    GRADE_CODE NUMBER,-- CONSTRAINT GRADE_CODE_FK REFERENCES USER_GRADE(GRADE_CODE) --컬럼레벨 FK지정
...
INSERT INTO USER_USED_FK
VALUES (2, 'USER02', 'PASS01', '홍길동', '남', '010-1234-1234', 'hong123@kh.or.kr', 10);

INSERT INTO USER_USED_FK
VALUES (3, 'USER03', 'PASS01', '홍길동', '남', '010-1234-1234', 'hong123@kh.or.kr', 30);

INSERT INTO USER_USED_FK
VALUES (4, 'USER04', 'PASS01', '홍길동', '남', '010-1234-1234', 'hong123@kh.or.kr', NULL);
--> NULL 사용 가능
INSERT INTO USER_USED_FK
VALUES (5, 'USER05', 'PASS01', '홍길동', '남', '010-1234-1234', 'hong123@kh.or.kr', 50);
--ORA-02291: 무결성 제약조건(HGH.GRADE_CODE_FK)이 위배되었습니다- 부모 키가 없습니다
            --> 참조 무결성 위배
--> 50이라는 값은 USER_GRADE 테이블 GRADE CODE 컬럼에서 제공하는 값이 아니므로
...
FROM USER_GRADE
WHERE GRADE_CODE = 10;
--ORA-02292: 무결성 제약조건(HGH.GRADE_CODE_FK)이 위배되었습니다- 자식 레코드가 발견되었습니다

-- 1) ON DELETE RESTRICTED (삭제 제한) 로 기본 지정되어 있음
-- FOREIGN KEY 지정된 컬럼에서 사용되고 이는 값일 경우
-- 제공하는 컬럼의 값은 삭제하지 못함
...
CREATE TABLE USER_USED_FK2(
    ...
    PHONE VARCHAR2(30),
    EMAIL VARCHAR2(50),
    GRADE_CODE NUMBER,-- CONSTRAINT GRADE_CODE_FK REFERENCES USER_GRADE2(GRADE_CODE) --컬럼레벨 FK지정
...
-- 부모 키 삭제 시 값을 사용하는 자식 테이블의 컬럼에 해당하는 행이 삭제가 됨
...
INSERT INTO USER_USED_CHECK
VALUES (2, 'USER02', 'PASS02', '홍길동', '남자', '010-1234-1234', 'hong123@kh.or.kr');
--ORA-02290: 체크 제약조건(HGH.GENDER_CHECK)이 위배되었습니다
--GENDER 컬럼에 CHECK 제약조건으로 '남'또는 '여'만 기록 가능한데
--'남자'라는 조건 이외의 값이 들어와서 에러 발생

--CHECK 제약 조건은 범위로도 설정 가능

-- 중요도 PK , FK> NOT NULL > UNIQUE > CHECK
...
CREATE TABLE USER_TEST(
    ...
    PNO VARCHAR(14) CONSTRAINT UK_PNO UNIQUE CONSTRAINT NN_PNO NOT NULL,
    GENDER VARCHAR2(10) CONSTRAINT CK_GENDER CHECK(GENDER IN ('남','여') ),
    PHONE VARCHAR2(30),
    ADDRESS VARCHAR2(50),
    STATUS VARCHAR2(5) CONSTRAINT NN_STATUS CHECK(STATUS IN ('Y','N') ) CONSTRAINT CK_STATUS NOT NULL
);
...
INSERT INTO USER_TEST VALUES (1, 'USER01', 'PASS01', '881001-1234567', '남', '010-123-4567','집', 'Y');
INSERT INTO USER_TEST VALUES (2, 'USER02', 'PASS02', '891001-1234567', '남', '010-344-4567','숙소', 'Y');
INSERT INTO USER_TEST VALUES (3, 'USER03', 'PASS03', '871001-2234567', '여', '010-455-4567','식당', 'N');
INSERT INTO USER_TEST VALUES (4, 'USER04', 'PASS04', '861001-1234567', '남', '010-566-4567','밥', 'N');
INSERT INTO USER_TEST VALUES (5, 'USER05', 'PASS05', '851001-2234567', '여', '010-788-4567','회사', 'Y');
...
--3) 테이블 형태(컬렴명, 데이터 타입)만 복사 (데이터 복사X)
...
CONSTRAINT FK_NATIONAL_CODE FOREIGN KEY(NATIONAL_CODE) REFERENCES NATIONAL;