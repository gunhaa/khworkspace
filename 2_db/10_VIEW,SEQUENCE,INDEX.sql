...
--ORA-01400: NULL을 ("HGH"."DEPT_COPY2"."DEPT_TITLE") 안에 삽입할 수 없습니다
-- == INSERT INTO DEPT_COPY2 V_DCOPY2 VALUES ('D0', NULL ,'L3'); 와 같은 의미이다.

-- VIEW를 이용한 INSERT 시 원본 테이블에 삽입이 된다.
--> 원본 테이블 삽입 시 VIEW INSERT 구문이 미포함된 커럶에는 NULL 이 저장된다.
--> 그런데 DEPT_TITLE 컬럼에 NOT NULL 제약조건이 설정되어 있어서 오류 발생!
--> VIEW를 이용한 INSERT 실패

-- 결론 : VIEW를 가지고 DML 하지말자
...
-- 시퀀스는 롤백을 수행한다고 해서 처음 값으로 돌아가지 않음
--> 계속 증가된 상태를 유지
ROLLBACK;
SELECT * FROM EMPLOYEE_COPY4;
SELECT SEQ_EMP_ID.CURRVAL FROM DUAL;
