...
* 사용자 계정 : 데이터베이스에 대하여 질의, 갱신, 보고서 작성 등의
                ...
                ex) jhs계정(각자 이니셜 계정), updown, workbook 등
*/
...
-- ROLE : 권한의 묶음
-->묶어둔 권한(ROLE)을 특정 계정에 부여 --> 해당 계정은 트정 권한을 가진 역할을 갖게 된다.
...
--2) RESOURCE : DB 사요을 위한 기본 객체 생성 권한 묶음
--[SYS 관리자 계정]
...
--[SYS 관리자 계정]
GRANT CONNECT , RESOURCE TO hgh_sample;

--------------------------------------------------------------------------------

--객체 권한
--> hgh_sample 계정에서 hgh계정의 EMPLOYEE 테이블 조회

-- 1. [샘플 계정] hgh 계정의 EMPLOYEE 테이블 조회

SELECT * FROM hgh.EMPLOYEE; --> EMPLOYEE 테이블(객체) 조회 권한 없음

-- 2. [이니셜 계정] 샘플 계정에 EMPLOYEE 테이블 조회 권한 부여
GRANT SELECT ON EMPLOYEE TO hgh_sample; -- Grant을(를) 성공했습니다.

--3. [샘플 계정] 다시 hgh 계정의 EMPLOYEE 테이블 조회
SELECT * FROM hgh.EMPLOYEE;

-- 4. [이니셜 계정] 부여했던 SELECT 권한 회수 
REVOKE SELECT ON EMPLOYEE FROM hgh_sample; --Revoke을(를) 성공했습니다.

--5. [샘플 계정] 다시 hgh 계정의 EMPLOYEE 테이블 조회
SELECT * FROM hgh.EMPLOYEE; --> 조회 안됨
