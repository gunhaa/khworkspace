...
/*
    *SUBQUERY(서브쿼리)
    - 하나의 SQL문 안에 포함된 또 다른 SQL(SELECT)
    - 메인쿼리(기존쿼리)를 위해 보조 역할을 하는 쿼리문
    -- SELECT, FROM ,WHERE , HAVING 절에서 사용 가능
*/

-- 서브쿼리 예시1, 
-- 부서 코드가 노옹철 사원과 같은 소속의 직원 이름, 부서코드 조회하기
...
-- 2) 직원들 중 급여가 3047663 이상인 사원의 사번, 이름, 직급코드, 급여 조회
...
FROM EMPLOYEE
GROUP BY DEPT_CODE);
--1) 부서 별 급여합 중 가장 큰 값 조회(부서별 급여합 1등)
SELECT MAX(SUM(SALARY))
FROM EMPLOYEE
GROUP BY DEPT_CODE;

--2) 부서별 급여 합이 1770000인 부서의 부서명과 급여 합 조회
SELECT DEPT_TITLE , SUM(SALARY)
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
GROUP BY DEPT_TITLE
HAVING SUM(SALARY) = 17700000;
--LEFT는 NULL 값을 보고싶어서 .EMPLOYEE를 기준으로 합쳐짐
...
-- 3-2) ANY를 이용하여 과장 중 가장 급여가 적은 인원을 초과하는 대리를 조회
...
-- 2)DEPARTMENT 테이블에서 위의 결과와 동일한 LOCATION_ID를 가지고 있는 
-- DEPT_ID를 조회
...
-- 3) 최종 적으로 EMPLOYEE테이블에서 위의 결과들과 동일한 DEPT_CODE를 가지는 사원을 조회
...
-- 2) 퇴사한 여직원과 같은부서 ,같은 직급인 사람을 구해보기
...
WHERE (DEPT_CODE,JOB_CODE) = (SELECT DEPT_CODE, JOB_CODE
 ...
 --첫번째 자리의 결과는 첫번째 것을 찾아가고 , 두번쨰 것은 두번째 것을 찾아간다.
...
JOIN JOB USING (JOB_CODE)
-- NATURAL JOIN JOB -- 자연 조인(컬럼명, 데이터 타입 일치하는 두 컬럼명 조회)
WHERE (DEPT_CODE, JOB_CODE)=(SELECT DEPT_CODE, JOB_CODE
                            FROM EMPLOYEE
                            WHERE EMP_NAME='노옹철')
AND EMP_NAME != '노옹철';
...
--다른 방법 WHERE EXTRACT(YEAR FROM HIRE_DATE) =2000 이용
...
-- 사번 ,이름, 직급, 급여를 조회하세요
-- 단, 급여와 급여평균은 만원 단위로 계산하세요, TRUNC(컬럼명, -4)
...
-- 5. 상[호연]관 서브쿼리(메인쿼리 1행씩 우선 해석, 서브 쿼리를 나중에 해석)
-- 상관 커리는 메인 쿼리가 사용하는 테이블의 값을 서브쿼리가 이용해서 결과를 만듦
...
SELECT EMP_ID ,ENT_YN, DEPT_CODE, EMP_NAME , NVL(DEPT_TITLE , '소속없음') , JOB_NAME, HIRE_DATE
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN JOB USING (JOB_CODE)
WHERE (DEPT_CODE, HIRE_DATE) IN (SELECT DEPT_CODE, MIN(HIRE_DATE)
...
-- 스칼라 (하나 라는 뜻)
SELECT EMP_NAME , SALARY , (SELECT FLOOR(AVG(SALARY)) FROM EMPLOYEE)
FROM EMPLOYEE;
...
--메인쿼리(MAIN)에서 MANAGER_ID를 얻어와 서브쿼리(SUB)에서 같은 EMP_ID를 가진 사원 이름을 조회
FROM EMPLOYEE MAIN;
...
-- 4) 1,2,3번을 토대로 급여 상위 5명 조회 
SELECT ROWNUM, EMP_NAME, SALARY
FROM EMPLOYEE
WHERE ROWNUM <= 5
ORDER BY 3 DESC;

--SELECT 해석 순서를 고려하지 않음 FROM , WHERE ,SELECT, ORDER 순
...
FROM EMPLOYEE
ORDER BY 2 DESC;
--> 해결1의 조회 결과(RESULT SET)을 가상의 테이블(==VIEW) 취급을 할 예정

-- 해결 2) 해결 1의 조회결과를 FROM절에 사용한 후 
--         상위 5행만 조회하기

SELECT ROWNUM, EMP_NAME, SALARY
FROM (SELECT EMP_NAME , SALARY 
        FROM EMPLOYEE
        ORDER BY 2 DESC) --FROM 절 내부에 포함된 가상의 테이블 == 인라인 뷰
WHERE ROWNUM <= 5
ORDER BY 3 DESC;

-- 급여 평균 3위 안에 드는 부서의 부서코드와 부서명, 평균 급여를 조회
...
WHERE ROWNUM <=3;
   ...
   --FROM 절 안에 들어간 테이블에서 SALARY의 정보가 없어서 에러가 나는 것
...
--                EX) 공동 1위가 2명이면 다음순위는 2위가 아니라 3위

SELECT RANK() OVER(ORDER BY SALARY DESC) 순위, EMP_NAME, SALARY
FROM EMPLOYEE;

-- DENSE_RANK() OVER : 동일한 순위 이후의 등수를 동일한 인원 수 만큼 건너뛰고 순위 계산
--                EX) 공동 1위가 2명이면 다음순위는 2위가 아니라 3위
...
WHERE (DEPT_CODE ,MANAGER_ID)=(SELECT DEPT_CODE ,MANAGER_ID
                              ...
                              AND SUBSTR(EMP_NO,8,1)='2');
--AND MAIN.MANAGER_ID=SUB.MANAGER_ID) 필요하지않나?? 다음에 볼때 다시
