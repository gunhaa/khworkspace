...
-- 직원 번호, 직원명, 부서코드 (EMPLOYEE)
-- 부서명 (DEMPARTMENT)을 조회하고자 할 때
...
WHERE DEPT_CODE = DEPT_ID; -- WHERE 절에 같은 값을 가지고 있는 컬럼을 작성
...
--EX) EMPLOYEE , DEPARTMENT 조인 시 하동운, 이오리 포함X
...
-- 3) FULL [OUTER] JOIN : 합치기에 사용한 두 테이블이 가진 모든행을 결과에 포함
-- ** 오라클 구문은 FULL OUTER JOIN 사용X
-- ANSI 표준방식
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE FULL OUTER JOIN DEPARTMENT ON(DEPT_CODE=DEPT_ID);

-- EMPLOYEE 테이블의 하동운, 이오리
-- DEPARTMENT 테이블의 마케팅부, 국내영업부, 해외영역3부 모두 결과에 포함
-- 오라클 구문
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE, DEPARTMENT
WHERE DEPT_CODE(+) = DEPT_ID(+);
...
--ANSI
-- 사수 이름 조회
SELECT E.EMP_ID, E.EMP_NAME, NVL(E.MANAGER_ID, '없음') AS "사수 번호", NVL(M.EMP_NAME, '없음') AS "사수 이름"
FROM EMPLOYEE E
...
-- 동일한 타입과 이름을 가진 칼럼이 있는 테이블간의 조인을 간단히 표현하는 방법
-- 반드시 두 테이블 간의 동일한 컬럼명, 타입을 가진 컬럼이 필요
--> 없을 경우 교차 조인이 됨
SELECT EMP_NAME, JOB_NAME
FROM EMPLOYEE
--JOIN JOB USING(JOB_CODE); -- 원래 정상적인 조인코드
NATURAL JOIN JOB;
...
-- 사번, 이름, 직급명, 부서명, 근무 지역명, 급여를 조회하시오
...
-- 2. 이름에 '형'자가 들어가는 직원의 사번, 사원명, 부서명을 조회하시오
...
-- 4. 보너스 포인트를 받는 직원의 사원명, 보너스 포인트, 부서명, 근무지역명을 조회하시오.
...
-- 6. 급여 등급별 최소 급여(MIN_SAL)를 초과해서 받는 직원들의
-- 사원명, 직급명,급여,연봉(보너스 포함)을 조회하시오
...
-- 8. 같은 부서에 근무하는 직원들의 사원명, 부서코드, 동료 이름을 조회하세요
--SELF JOIN 사용

SELECT E.EMP_NAME 사원명 , E.DEPT_CODE 부서코드, M.EMP_NAME 동료이름
FROM EMPLOYEE E
LEFT JOIN EMPLOYEE M ON(M.DEPT_CODE=E.DEPT_CODE)
WHERE E.EMP_NAME!= M.EMP_NAME
ORDER BY 부서코드;

--- 9. 보너스포인트가 없는 직원들 중에서
--직급코드가 J4와 J7 인 직원들의 사원명,직급명, 급여를 조회하시오
-- 단, JOIN , IN 사용할것
