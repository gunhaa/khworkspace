...
VALUES(900, '장채헌', '901123-1234567', 'jang_ch@kh.or.kr', '01012341234',
        'D1', 'J7', 'S1' , 9000000, 0.8, 200 , SYSDATE,NULL,'N');
...
COMMIT; -- 수행된 DML 결과를 DB에 반영할때 사용한다
...
--서브 쿼리 (SQL, 내부에 포함된 SELECT문)를 이용한 삽입(VALUES 생략 가능)
...
FROM DEPARTMENT2
WHERE DEPT_ID= 'D9';
--DEPARTMENT2 테이블에서 DEPT_ID가 'D9인 행의 DEPT_TITLE을 '전략 기획팀'으로 수정
...
--> 조건을 설정하지 않아 모든 행의 컬럼 값이 수정되는 문제 발생
--> WHERE 절 조건 잘 작성!!
...
--급여와 보너스율을 유재석 사원과 동일하게 변경해주기로 했다.
...
-- * 다중행 다중열 서브쿼리를 이용한 UPDATE 문

--EMPLOYEE2 테이블에서
--박명수 사원의 급여 인상 소식을 전해들은 다른 멤버들이
--단체로 파업을 진행했다
--노옹철, 전형돈,정중하,하동운 사원의 급여와 보너스를
--유재식 사원의 급여와 보너스가 같게 변경하는 UPDATE문을 작성하시오
...
-- EMPLOYEE2 테이블에서 이름이 '장채헌'인 사원의 정보 삭제
DELETE
FROM EMPLOYEE2
WHERE EMP_NAME='장채헌';
