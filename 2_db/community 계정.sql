...
-- 1번 게시판의 전체 게시글 수 조회
SELECT COUNT(*) FROM BOARD WHERE BOARD_CD=1 AND BOARD_ST='N';

-- 1번 게시판의 제목에 "게시글" 포함된 게시글 수 조회
SELECT COUNT(*) FROM BOARD WHERE BOARD_CD=1 AND BOARD_ST='N' AND BOARD_TITLE LIKE '%게시글%';
-- 1번 게시판의 내용에 "게시글" 포함된 게시글 수 조회
SELECT COUNT(*) FROM BOARD WHERE BOARD_CD=1 AND BOARD_ST='N' AND BOARD_CONTENT LIKE '%게시글%';
-- 1번 게시판의 제목과 내용에 "게시글" 포함된 게시글 수 조회
SELECT COUNT(*) FROM BOARD WHERE BOARD_CD=1 AND BOARD_ST='N' AND (BOARD_TITLE LIKE '%게시글%' OR BOARD_CONTENT LIKE '%게시글%');
-- 1번 게시판의 작성자에 "?" 포함된 게시글 수 조회
SELECT COUNT(*) FROM BOARD JOIN MEMBER USING(MEMBER_NO) WHERE BOARD_CD=1 AND BOARD_ST='N' AND MEMBER_NICK LIKE '%유저일%';
...
SELECT ROWNUM RNUM, A.* FROM (
        SELECT BOARD_NO, BOARD_TITLE, BOARD_CONTENT, MEMBER_NICK, TO_CHAR(CREATE_DT, 'YYYY-MM-DD') AS CREATE_DT,READ_COUNT ,
        (SELECT IMG_RENAME FROM BOARD_IMG WHERE IMG_LEVEL=0 AND BOARD_IMG.BOARD_NO =BOARD.BOARD_NO) THUMBNAIL
        FROM BOARD JOIN MEMBER USING(MEMBER_NO) WHERE BOARD_CD= 1 AND BOARD_ST ='N'
        --condition
...
-----------------------------------
        ...
        SELECT BOARD_NO, BOARD_TITLE, BOARD_CONTENT, MEMBER_NICK, TO_CHAR(CREATE_DT, 'YYYY-MM-DD') AS CREATE_DT,READ_COUNT ,
        (SELECT IMG_RENAME FROM BOARD_IMG WHERE IMG_LEVEL=0 AND BOARD_IMG.BOARD_NO =BOARD.BOARD_NO) THUMBNAIL
        FROM BOARD JOIN MEMBER USING(MEMBER_NO) WHERE BOARD_CD= 1 AND BOARD_ST ='N' AND BOARD_TITLE LIKE '%게시글%'
        ORDER BY BOARD_NO DESC
		    )A
		)
		WHERE RNUM BETWEEN 1 AND 10