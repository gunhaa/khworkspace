...
--               ADD [CONSTRAINT 제약조건명] 제약 조건(컬럼명) [REFERENCS 테이블명 [(컬럼명)];

-- 제약 조건 삭제 : ALTER TABLE 테이블명
--                DROP CONSTRAINT 제약조건명;


-- 서브 쿼리를 이용해서 DEPARTMENT 테이블 복사 --> NOT NULL 제약조건만 복사됨
...
--DEPT_COPY 테이블에 DEPT_TITLE 칼럼에 UNIQUE 제약ㅈ건 추가 (제약 조건명 DEPT_TITLE_U)

ALTER TABLE DEPT_COPY
ADD CONSTRAINT DEPT_TITLE_U UNIQUE(DEPT_TITLE);

--DEPT_COPY 테이블의 LOCATION_IN 컬럼에 CHECK 제약조건 추가
...
--> NOT NULL을 제이한 제약 조건은 추가적인 조건으로 인식됨. (ADD/DROP)
--> NOT NULL 은 기존 컬럼의 성질을 변경하는 것으로 인식됨. (MODIFY)
ALTER TABLE DEPT_COPY
...
ALTER TABLE DEPT_COPY
MODIFY DEPT_ID CONSTRAINT SYS_C008505 NULL;
--     (컬럼이름)
...
-- (기본값 수정)
-- DEPT_COPY 테이블의 LNAME 기본 값을 '한국' -> '대한민국'으로 변경

ALTER TABLE DEPT_COPY MODIFY LNAME DEFAULT '대한민국';

SELECT * FROM DEPT_COPY; --> 기본 값을 변경했다고해서, 기존에 저장된 값이 변경되진 않음
...
--> 제약 조건까지 모두 삭제
-- == FK 제약조건으로 인해 원래는 삭제가 불가능 하지만, 제약 조건을 없애버려서 FK 관계를 해제
