...
public class PolyService {
	...
	public void ex1() {
		...
		car2.setFuel("전기");
		car2.setWheel(4);
		//car2.setBatteryCapacity(1000000); //오류발생
		...
		Car [] arr= new Car[3]; //부모 타입 참조 변수 배열을 선언 및 할당
		//각 배열 요소가 Car타입 참조 변수

		arr[0] = car; // Car 주소 == Car 객체 (Car 객체를 참조한다.)
		//Car 참조 변수

		arr[1] = car2; //Tesla 주소 == Tesla 객체 ( tesla 객체를 참조한다.)
		//Car 참조 변수
	...
	public void ex2() {
		...
		//arr[1]이 참조하는 객체가 Tesla이면 True, 아니면 false 를 반환
		System.out.println(arr[1] instanceof Tesla); //true

		//arr[1]이 참조하는 객체가 Spark이면 True, 아니면 false 를 반환
		System.out.println(arr[1] instanceof Spark); //false

		//arr[1]이 참조하는 객체가 Car이면 True, 아니면 false 를 반환
	...
	//매개 변수에 부모 타입 참조변수를 작성하면 모든 자식 객체를 전달 받을 수 있다.
	public void printCar(Car temp) {
		//매개 변수에 작성된 참조형 변수에는 주소가 저장된다. (얕은 복사)
		//메소드 내부 변수 + 매개변수 == 지역변수(Local Variable)

		System.out.println("엔진 : "+temp.getEngine());
		System.out.println("엔진 : "+temp.getFuel());
		System.out.println("엔진 : "+temp.getWheel());
		System.out.println();
	}

	//전달 받은 매개변수에 따라서 Car또는 자식 객체를 생성하고
	//생성된 객체의 주소를 반환

	public Car createCar(int num) {

		Car result = null;
		// null == 참조형 변수가 선언 되었으나 아무것도 참조하고 있지 않음
		...
		// 반환형은 Car이지만
		//case 2,3 이면 Car의 자식 객체의 주소가 반환된다.
	...
	public void ex4() {
		...
		// !!!!!!!!!!다운 캐스팅 주의 사항!!!!!!!!!!!!
		// 다운 캐스팅 - 업 캐스팅을 했을때 쓰지 못한 부분을 쓰기 위해 사용하는 기술
		Car c1 = new Tesla();

		//Spark s1 = (Spark)c1; // 다운 캐스팅
		// java.lang.ClassCastException 형변환 예외 발생, 에러 뜸
		// -> c1이 참조하는 객체는 Tesla 인데
		//    Spark 참조변수로 Tesla를 참조하려고해서 문제 발생

		// @@@ 해결 방법 : instance와 같이 사용! / 오류 방지를 위해 확인하는 것
	...
	public void ex5() {
		...
		//하지만 실행해보면 자식(Spark)의 toString()이 호출 되는 것을 확인
		// -> 컴파일 시에는 부모(Car)와 바인딩 == [정적 바인딩]
		// -> "실행 시" 에는 자식(Spark)의 오버라이딩 된 메소드와 바인딩 ==[동적바인딩]

		// ** 동적 바인딩 활용 방법 **

		Car[] arr = {
				new Car("v6 엔진", "경유", 12),
				new Tesla("전기모터", "전기", 4, 5000000),
