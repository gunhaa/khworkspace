...
public class ArrayExample2 {
	...
	public void deepcopy() {
		...
		for(int i=0 ; i<arr.length; i++) {
			copyArr1[i]=arr[i]; //오른쪽 값을 왼쪽에 대입한다
		}
	...
	public void createLottoNumber() {
		...
		for(int i=0; i<lotto.length; i++) { //1부터 45사이의 난수 생성
			...
			for(int x=0; x<i; x++) {
				if(lotto[i]==lotto[x]) {
					...
					//--> i는 0~5까지 6회 반복되지만 
					//    i 값을 인위적으로 1감소시켜서 추가로 반복되는 모양을 만듬
		...
		//	 arrays.sort(배열명) : 배열 내 값들이 오름 차순으로 정렬됨
		
		
		Arrays.sort(lotto); //배열 오름 차순 정렬
